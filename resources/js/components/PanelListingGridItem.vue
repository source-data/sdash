    ...
    <li :id="itemId" class="sd-grid-item" :class="{ 'sd-grid-item__expanded' : isExpanded }">
        ...
        <div class="sd-grid-image-container" >
            ...
            <footer class="sd-grid-item--image-footer" :id="'scroll-anchor-' + panelId">
                ...
                <font-awesome-icon :class="panelAccessReason" icon="exchange-alt"  v-if="panelAccessReason=='group'" title="Shared with group"/>
            </footer>
            <div class="css_arrow" v-if="isExpanded"></div>
        </div>

        <div class="sd-grid-extra" id="panel-detail" ref="listGridExtra" v-if="isExpanded">
            <button type="button" aria-label="Close" class="close sd-grid-extra--close" @click.prevent="toggleExpanded"><span aria-hidden="true">&times;</span></button>
...
export default {
    ...
    computed: {
        ...
        panelAccessReason(){
            ...
            if(!thisPanel.made_public_at && thisPanel.groups.length === 0 && thisPanel.user_id === this.currentUser.id) return 'private'
    ...
    methods:{ //run as event handlers, for example
        ...
        toggleExpanded(){
            ...
            if(this.isExpanded) {
                ...
                this.$scrollTo('#scroll-anchor-' + this.panelId, 500, {offset: -60})
            }
        },
        toggleSelected(){
...
<style lang="scss">
    ...
    $sd-extra-height: 760; // info box height in pixels
