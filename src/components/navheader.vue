...
<template>
  ...
  <b-navbar toggleable="md" type="dark" variant="primary" fixed="top" >
    ...
    <b-navbar-brand href="#">
      <!-- <img src="../assets/sib_logo_small.gif" style="margin-right:5px" >     -->
      <router-link to="/" style="font-size: 1.3rem;" > {{siteTitle}} </router-link>
    </b-navbar-brand>
      ...
      <b-navbar-nav class="ml-auto">
        <b-navbar-nav right>
          ...
          <b-nav-item v-access="'active'">
            {{ $t('welcome') }} <router-link to="/user"> {{ user.fullname }} </router-link>
          </b-nav-item>
          <b-nav-item v-access="'active'">
            <a class="pointer" @click="logout()" > <v-icon name="sign-out-alt" /> </a>
          </b-nav-item>
