...
class DownloadController extends Controller
{
    ...
    public function downloadPdf(Panel $panel, Request $request)
    {
        ...
        if( !Gate::allows('access-panel', $panel)) throw new Exception("Access denied.", 401);
    ...
    public function downloadZip(Panel $panel, Request $request)
    {
        ...
        try {
            ...
            $panel->load(['image', 'files' => function($query){ $query->where('is_archived', false)->where('type', 'file'); }]);
    ...
    public function downloadDar(Panel $panel, Request $request)
    {
        ...
        try {
            ...
            $panel->load(['image', 'user', 'files' => function($query){ $query->where('is_archived', false); }, 'tags' => function($query){ $query->withPivot(['id', 'origin','role','type','category']);}]);
